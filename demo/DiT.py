...
MODEL_NAME = f"dit_cfg_{N_LAYER}_LAYERs_{N_HEAD}_HEADs_{N_EMBD}_EMBD_DIM_{T_MAX}_TMAX_{DATASET}"
...
class DiT(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 2+self.num_patches, n_embd)) ## original DiT paper uses 2D cos/sin embeddings
    ...
    def forward(self, x, t, label):
        ...
        x, _ = pack((te, lab, x), 'B * n_embd') # expand into dim=1 to form num_patches+2
...
def make_denoise_frames(model, label = None, num_rows = 1, num_steps = 20, scale=SAMPLE_GUIDANCE, neg_label=UNCOND_LABEL):
  ...
  if label is None:
     ...
     labels = torch.full((N_CLASS,), label, dtype=torch.long, device=device).repeat(num_rows)
  model.eval()
  noise = torch.randn(N_CLASS*num_rows, IMG_CHANNELS, IMG_SIZE, IMG_SIZE, device=device)
  diffused, history = model.ddpm.backward(noise, model, labels, method='cfg', scale=scale, neg_label=neg_label, steps = num_steps)
  ...
  for i in range(len(np_frames)):
    ...
    ax.text(0, 1.23, frame_text, fontsize=10, va='top', ha='left', transform=ax.transAxes)
    ax.set_axis_off()
    images.append(fig)
  return images
...
def plot_MNIST_images(num_rows, label):
  ...
  indices = np.arange(len(images_to_plot)).reshape(N_CLASS, num_rows).T.flatten().tolist()
