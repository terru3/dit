...
path = "./"
CASEY_MODEL = (
    f"{path}/model/dit_cfg_8_LAYERs_32_HEADs_256_EMBD_DIM_400_TMAX_MNIST_epoch_29.pt"
)
...
def main():
    ...
    model.load_state_dict(
        torch.load(CASEY_MODEL, map_location=device)["model_state_dict"]
    )
    print("Model loaded")
    ...
    with col1:
        generate_option = st.radio(
            "Choose some numbers to generate:",
            ["**Specific Digit**", ":rainbow[ALL THE NUMBERS]"],
            captions=["Choose from 0 through 9.", "All of them."],
            horizontal=True,
        )
    with col2:
    ...
    with st.expander("Diffusion Parameters (advanced settings 🤠)", expanded=False):
        st.write(
            "Set parameters for the denoising process. Mouse over the '?' symbol to learn about each parameter."
        )
        st.markdown("##### Output batch size (Number of rows to generate)")
        num_rows = st.number_input(
            "Choose the number of images to generate (10 * number of rows)",
            min_value=1,
            max_value=8,
            value=3,
            step=1,
        ...
        negative_label = st.selectbox(
            "Select the negative label condition",
            options=negative_label_options,
            index=len(negative_label_options) - 1,
    ...
    if st.button("Generate Digits!"):
        with st.spinner("🍳😤 Cooking..."):
            st.session_state.frames = make_denoise_frames(
                model,
                label=label,
                num_rows=num_rows,
    ...
    if st.session_state.frames is not None:
        ...
        frame_index = st.slider(
            "Select a specific frame of the denoising process",
            0,
        ...
        if animate_button:
            for i in range(len(st.session_state.frames)):
                ...
                time.sleep(
                    0.1
                )  # Add delay for animation effect (100 ms between frames)
    ...
    st.caption(
        "UCLA DSU Project Winter 2024: Daniel Mendelevitch, Terry Ming, Casey Tattersall, Sean Tjoa"
    )
    st.caption(
        "Data Attribution: LeCun, Yann, Corinna Cortes, and C. J. C. Burges. \
